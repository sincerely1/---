...
import { get_type_summary_data } from '@/api/chart'
...
export default {
  ...
  methods: {
    async get_data(course_id) {
      ...
      const res = await get_type_summary_data(course_id, store.getters.account)
      const res_data = res.data
      this.option.xAxis.data = res_data.type
      this.option.series[0].data = res_data.answer_question
      this.option.series[1].data = res_data.correct_question
      this.option.series[2].data = res_data.commit_count
      this.chart.clear()
